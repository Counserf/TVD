...
- **Настройка отступления**: Измените `_retreatRatio` (от 0 до 1) для определения уровня потерь, при котором игроки смогут отступать. Например, `0.5` означает, что отступление возможно при половине потерь, а `0.2` — при больших потерях (остаток войск - 20%).
